...
export default function LinkCard({ link, onChat, refreshLinks, isSharedView = false }: LinkCardProps) {
  ...
  if (link.platform === 'X' && link.embedHtml) {
    return (
      <motion.div 
        // FIX: Removed fixed height classes (min-h-[400px] max-h-[400px])
      ...
      >
        {/* The blockquote will now determine the card's height */}
  ...
  return (
    ...
    >
      ...
      {/* Action buttons */}
      {!isSharedView && (
        <div className="p-4 bg-stone-50 border-t border-stone-200 flex justify-between items-center mt-auto flex-shrink-0">
          <button 
            onClick={() => onChat?.(link)} 
            className="flex items-center gap-2 text-sm text-sky-600 font-semibold hover:text-sky-800 transition-colors"
          >
